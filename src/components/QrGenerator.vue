...
async function loadRecent() {
  try {
    ...
    recentError.value = '';
  const { data } = await axios.get('/api/documents', { params: { page: page.value, limit: limit.value } });
...
async function deleteRecent(item) {
  try {
    await axios.delete(`/api/documents/${item.docId}`);
...
async function loadTrash() {
  try {
    const { data } = await axios.get('/api/trash', { params: { page: trashPage.value, limit: 10 } });
...
async function restore(item) {
  try {
    await axios.post(`/api/trash/${item.docId}/restore`);
...
function deleteForever(item) {
  confirm(`Permanently delete "${item.originalName || item.docId}"? This cannot be undone.`, async () => {
    try {
      await axios.delete(`/api/trash/${item.docId}`);
