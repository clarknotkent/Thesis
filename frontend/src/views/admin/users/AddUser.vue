...
const handleSubmit = async (formData) => {
  ...
  try {
    // Determine if userIdentifier is email or username and set both fields appropriately
    let username = ''
    let email = ''
    if (formData.userIdentifier.includes('@')) {
      // User entered an email - use it as-is, extract username from before @
      email = formData.userIdentifier
      username = formData.userIdentifier.split('@')[0]
    } else {
      // User entered a username - append @immunizeme.com to create email
      username = formData.userIdentifier
      email = `${formData.userIdentifier}@immunizeme.com`
    }
    ...
    const payload = {
      ...
      // employee_id is autogenerated for admin and health_staff
      ...(formData.role !== 'admin' && formData.role !== 'health_staff' && { employee_id: formData.employeeId || null }),
