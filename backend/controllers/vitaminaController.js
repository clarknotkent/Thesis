...
const listVitamina = async (req, res) => {
  try {
    const supabase = getSupabaseForRequest(req);
    const items = await vitaminaModel.listVitamina({}, supabase);
    res.json(items);
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Failed to fetch vitamina records' });
  }
};

const getVitaminaRecord = async (req, res) => {
  try {
    const supabase = getSupabaseForRequest(req);
    const item = await vitaminaModel.getVitaminaById(req.params.id, supabase);
    if (!item) return res.status(404).json({ message: 'Vitamina record not found' });
    res.json(item);
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Failed to fetch vitamina record' });
  }
};

const createVitaminaRecord = async (req, res) => {
  try {
    const supabase = getSupabaseForRequest(req);
    ...
    if (outside) {
      if (!payload.patient_id) return res.status(400).json({ success:false, message:'patient_id is required for outside vitamina' });
      if (payload.inventory_id) return res.status(400).json({ success:false, message:'inventory_id is not allowed for outside vitamina' });
      ...
      const row = await vitaminaModel.createVitamina(insertData, supabase);
    ...
    if (!visit_id) {
      if (!patient_id) return res.status(400).json({ success:false, message:'visit_id or patient_id is required for in-facility vitamina' });
  ...
  const vaccineId = await resolveVaccineId(supabase, { inventory_id: payload.inventory_id, vaccine_id: payload.vaccine_id });
    ...
    console.error(error);
    return res.status(500).json({ success:false, message:'Failed to create vitamina record' });
  }
};

const updateVitaminaRecord = async (req, res) => {
  try {
    const supabase = getSupabaseForRequest(req);
    const data = await vitaminaModel.updateVitamina(req.params.id, req.body, supabase);
    if (!data) return res.status(404).json({ success:false, message:'Vitamina record not found' });
    res.json({ success:true, data });
  } catch (error) {
    console.error(error);
    res.status(500).json({ success:false, message: 'Failed to update vitamina record' });
  }
};

const deleteVitaminaRecord = async (req, res) => {
  try {
    const supabase = getSupabaseForRequest(req);
    await vitaminaModel.deleteVitamina(req.params.id, supabase);
    res.status(204).send();
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Failed to delete vitamina record' });
  }
};
