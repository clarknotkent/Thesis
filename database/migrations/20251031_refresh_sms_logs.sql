...
-- 4) Backfill combined messages with vaccine names, dose numbers, guardian info (clean layout)
WITH base AS (
  SELECT
    ps.patient_id,
    ...
    string_agg(
      format('%s — Dose %s', COALESCE(v.antigen_name, 'vaccine'), COALESCE(ps.dose_number, 1)),
      E',\n' ORDER BY v.antigen_name, ps.dose_number
    ) AS vaccine_lines
  FROM patientschedule ps
  JOIN patients p ON p.patient_id = ps.patient_id  -- join to get guardian_id
  JOIN patients_view pv ON pv.patient_id = ps.patient_id
  LEFT JOIN vaccinemaster v ON v.vaccine_id = ps.vaccine_id
  WHERE ps.is_deleted = false
    AND ps.scheduled_date::date >= current_date
...
-- Map to trigger type and pick template (if any)
with_trigger AS (
  SELECT
    r.*,
    CASE r.days_until
...
-- Pick latest active template per trigger
tpl_pick AS (
  SELECT
    w.*,
    t.id AS template_id,
...
-- Render message text with variables replaced and clean fallback
render AS (
  SELECT
    p.patient_id,
    p.guardian_id,
      ...
      replace(
        replace(
          replace(
                      ...
                      replace(
                        COALESCE(
                          ...
                          format(
                            ...
                            to_char(p.sched_date, 'FMMonth DD, YYYY'),
                            COALESCE(p.vaccine_lines, '')
                          )
                        ),
            ...
            '{vaccine_lines}', COALESCE(p.vaccine_lines, '')
          ),
          '{scheduled_date}', to_char(p.sched_date, 'FMMonth DD, YYYY')
        ),
        '{appointment_date}', to_char(p.sched_date, 'FMMonth DD, YYYY')
      ),
