...
-- 4) Backfill combined messages with vaccine names, dose numbers, and guardian titles
WITH base AS (
  SELECT
    ps.patient_id,
    ...
    pv.guardian_relationship,
    CASE 
      WHEN lower(pv.guardian_relationship) = 'mother' THEN 'Ms.'
      WHEN lower(pv.guardian_relationship) = 'father' THEN 'Mr.'
      ELSE ''
  ...
  GROUP BY ps.patient_id, ps.scheduled_date::date, pv.full_name, pv.guardian_contact_number, pv.guardian_relationship
),
d AS (
